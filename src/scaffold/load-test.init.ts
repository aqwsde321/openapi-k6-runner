...
function renderReadme(moduleName: string, directory: string, cliPath: string | undefined): string {
  ...
  return [
    ...
    `| 5 | \`{{env.NAME}}\`을 쓰는 경우 \`${envPath}\` 작성 | - | \`${envPath}\` |`,
    ...
    '```',
    '',
    `\`run.sh\`가 실행할 때 \`${envPath}\`를 자동으로 export합니다. 이 파일은 \`run.sh\`와 같은 폴더에 있어야 합니다.`,
    ...
    `8. For scenarios that need secrets, copy \`${directory}/.env.example\` to \`${directory}/.env\` and fill local values.`,
