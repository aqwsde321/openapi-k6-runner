...
function renderReadme(moduleName: string, directory: string, cliPath: string | undefined): string {
  ...
  return [
    ...
    `로그 파일: \`${logPath}\``,
    '',
    'k6 기본 Web Dashboard를 사용하려면 k6 표준 환경변수를 붙입니다.',
    '',
    '```bash',
    `K6_WEB_DASHBOARD=true K6_WEB_DASHBOARD_OPEN=true ${runScriptArg} smoke`,
    '```',
    '',
    '각 scenario step의 시작/종료 로그까지 남기려면 `OPENAPI_K6_TRACE=1`을 함께 사용합니다.',
    '',
    '```bash',
    `OPENAPI_K6_TRACE=1 K6_WEB_DASHBOARD=true K6_WEB_DASHBOARD_OPEN=true ${runScriptArg} smoke --log`,
    '```',
    '',
