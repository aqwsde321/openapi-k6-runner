...
function renderReadme(moduleName: string, directory: string, cliPath: string | undefined): string {
  ...
  return [
    ...
    `| 4 | \`{{env.NAME}}\`을 쓰는 경우 \`${envPath}\` 작성 | \`${usesDefaultDirectory ? 'openapi-k6 test -s <name>' : `openapi-k6 test --config ${configArg} --module ${moduleName} --scenario ${shellQuote(scenarioTemplatePath)}`}\` | step별 API 검증 결과 |`,
    `| 5 | - | \`${usesDefaultDirectory ? 'openapi-k6 generate -s <name>' : `openapi-k6 generate --config ${configArg} --module ${moduleName} --scenario ${shellQuote(scenarioTemplatePath)} --write ${shellQuote(outputTemplatePath)}`}\` | \`${outputTemplatePath}\` |`,
    `| 6 | - | \`${runScriptArg} <name> --log\` | k6 부하 테스트 실행, \`${directory}/logs/<name>.log\` |`,
    '',
    ...
    '',
    `\`${catalogPath}\`에서 테스트할 endpoint의 \`operationId\`, \`method\`, \`path\`, \`parameters\`, \`hasRequestBody\`, \`requestBodyContentTypes\`를 확인합니다.`,
    '',
    ...
    '```',
    '',
    `\`${envPath}\`가 있으면 \`{{env.NAME}}\` template 값과 \`BASE_URL\`을 읽습니다. 현재 shell 환경변수가 같은 이름으로 있으면 shell 값이 우선합니다.`,
    '',
    '### 2-4. k6 스크립트 생성',
    '',
    '```bash',
    ...
    '```',
    '',
    `\`run.sh\`는 자신과 같은 폴더의 \`.env\`(\`${envPath}\`)만 자동으로 로드한 뒤 \`generated/<scenario>.k6.js\`를 실행합니다.`,
    ...
    '',
    '```bash',
    `${runScriptArg} smoke --report --duration 10s --vus 1`,
    '```',
    '',
    ...
    '',
    '```bash',
    `${runScriptArg} smoke --trace --log --report --duration 10s --vus 1`,
    '```',
    '',
    ...
    '## 3. 비밀 값 사용',
    '',
    `시나리오에서 \`{{env.NAME}}\`을 사용한다면 \`${directory}/.env.example\`을 \`${envPath}\`로 복사한 뒤 비밀 값을 채웁니다.`,
    '',
    '```bash',
    ...
    '```',
    '',
    `\`openapi-k6 test\`와 \`run.sh\`가 \`${envPath}\`를 읽습니다. 이 파일은 \`run.sh\`와 같은 폴더에 있어야 합니다.`,
    '백엔드 프로젝트 루트의 `.env`는 자동으로 읽지 않습니다. 루트 `.env` 값을 쓰려면 필요한 키만 이 파일로 복사하거나, 실행 전에 shell에서 직접 export합니다.',
    `\`${directory}/.gitignore\`가 \`.env\`를 무시하므로 실제 값은 commit하지 않습니다.`,
    '',
    '## 4. 자주 하는 수정',
    '',
    ...
    `7. For scenarios that need secrets, copy \`${directory}/.env.example\` to \`${directory}/.env\` and fill local values.`,
    ...
    `10. Run the generated script with \`${runScriptArg} <scenario>\` or the directory-specific run command shown above.`,
    '',
    '### Rules',
    '',
    ...
    `- Store real secret values in \`${directory}/.env\` and do not commit it. Keep placeholders only in \`${directory}/.env.example\`.`,
    ...
    '```',
    '',
    `Multipart file paths are relative to \`${directory}/\`. Put local upload fixtures under \`${directory}/fixtures/\` by default.`,
    ...
    `- \`${directory}/.env.example\`: example file for \`${directory}/.env\` secret values`,
    `- \`${directory}/run.sh\`: k6 runner that auto-loads \`${directory}/.env\` values`,
    ...
    `Do not edit ${directory}/README.md, ${directory}/run.sh, ${directory}/.env.example, or ${directory}/.gitignore unless explicitly asked to change scaffold files.`,
    ...
    `Use {{env.NAME}} for secrets and keep real values in ${directory}/.env only.`,
    'Validate the scenario using the command documented in this README.',
    'Then generate the k6 script using the new scenario generation command in this README.',
    `Do not edit ${directory}/README.md, ${directory}/run.sh, ${directory}/.env.example, or ${directory}/.gitignore unless explicitly asked to change scaffold files.`,
    'Keep human-facing documentation in Korean. Keep AI instruction sections in English.',
