...
function renderRunScript(): string {
  return [
    ...
    '  This script loads .env from the same directory before running k6.',
    '  See load-tests/README.md for the full workflow.',
    'EOF',
    '}',
    '',
    'while [[ $# -gt 0 ]]; do',
...
function renderReadme(moduleName: string, directory: string, cliPath: string | undefined): string {
  ...
  return [
    ...
    '```',
    '',
    '`run.sh`는 같은 폴더의 `.env`를 자동으로 로드한 뒤 `generated/<scenario>.k6.js`를 실행합니다. 빠른 사용법은 `run.sh --help`로 확인할 수 있습니다.',
    '',
