...
describe('README usage guide', () => {
  it('keeps the root entrypoint scannable and delegates detailed work to generated README', async () => {
    ...
    expect(readme).toContain('| 3 | `npx --yes openapi-k6 catalog --query <검색어>` | scenario에 쓸 endpoint 후보 검색 |');
    expect(readme).toContain('| 4 | `load-tests/scenarios/*.yaml` 작성 | catalog 기준으로 API 흐름 작성 |');
    expect(readme).toContain('| 5 | `npx --yes openapi-k6 validate -s <scenario-name>` | OpenAPI snapshot 기준 정적 검증 |');
    expect(readme).toContain('| 6 | `npx --yes openapi-k6 test -s <scenario-name>` | Node.js에서 scenario 1회 실행 검증 |');
    expect(readme).toContain('| 7 | `npx --yes openapi-k6 run -s <scenario-name> --log -- --vus 1` | 정적 검증, k6 스크립트 생성, k6 실행 |');
    expect(readme).toContain('| 8 | `npx --yes openapi-k6 generate -s <scenario-name>`, `./load-tests/run.sh <scenario-name> --log` | 스크립트만 생성하거나 runner로 실행 |');
    expect(readme).toContain('`<검색어>`는 catalog 검색어이고, `<scenario-name>`은 `load-tests/scenarios/<scenario-name>.yaml`의 이름입니다.');
    expect(readme).toContain('예시 값은 `login`, `smoke`입니다.');
    ...
    expect(readme).toContain('npx --yes openapi-k6 catalog --tag <tag>');
    expect(readme).toContain('예: 로그인 API는 `--query login`, auth tag는 `--tag auth`로 찾습니다.');
    ...
    expect(readme).toContain('`--query <검색어>`: catalog 전체에서 검색어가 들어간 API를 찾습니다.');
    expect(readme).toContain('`--tag <tag>`: OpenAPI에서 해당 tag로 묶인 API만 봅니다.');
    ...
    expect(readme).toContain('명령의 `-s <scenario-name>`은 scenario 파일 이름에서 확장자를 뺀 값입니다.');
    ...
    expect(readme).toContain('예시처럼 `{{env.*}}`를 쓰면 `test` 전에 `load-tests/.env`에 실제 값을 둡니다.');
    ...
    expect(readme).toContain('npx --yes openapi-k6 generate -s <scenario-name>');
    ...
    expect(readme).toContain('| scenario용 endpoint 검색 | `npx --yes openapi-k6 catalog --query <검색어>` |');
    ...
    expect(readme).toContain('npx --yes openapi-k6 catalog --query <검색어>로 테스트할 endpoint 후보를 확인해.');
