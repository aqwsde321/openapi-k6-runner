...
describe('openapi-k6 CLI', () => {
  ...
  it('initializes a load-tests scaffold in the target project', async () => {
    ...
    expect(readme).toContain('| AI에게 맡기기 | 아래 `AI에게 작업 맡기기` 프롬프트를 복사해서 agent에 전달합니다. |');
    expect(readme).toContain('| 직접 실행하기 | `사람이 직접 실행할 때`의 빠른 시작을 순서대로 실행합니다. |');
    ...
    expect(readme).toContain('대화형 `init`은 먼저 API 기본 주소를 입력받습니다.');
    ...
    expect(readme).toContain('`config.yaml`에 TODO가 남아 있으면 실제 API 정보로 채웁니다.');
    expect(readme).toContain('`init` 직후 config는 단일 서버용 기본 module만 들어 있습니다.');
    expect(readme).toContain('init 때 등록한 서버는 그대로 두고, 추가로 필요한 서버만 CLI로 추가합니다.');
    ...
    expect(readme).toContain('catalog에서 endpoint 후보 검색 후 scenario 작성');
    ...
    expect(readme).toContain('##### A. Endpoint 찾기');
    ...
    expect(readme).toContain('Catalog: load-tests/openapi/pharma.catalog.json');
    ...
    expect(readme).toContain('백엔드 OpenAPI 문서에 있는 `operationId`를 `sync`가 `load-tests/openapi/pharma.catalog.json`에 복사합니다.');
    ...
    expect(readme).toContain('##### B. Scenario 파일 선택');
    expect(readme).toContain('`load-tests/scenarios/smoke.yaml`는 `init`이 만든 기본 예시 scenario입니다.');
    expect(readme).toContain('처음 동작 확인은 이 파일을 수정해도 되고, 실제 업무 흐름은 `load-tests/scenarios/<name>.yaml`처럼 새 파일로 만들어도 됩니다.');
    expect(readme).toContain('##### C. 최소 YAML 작성');
    ...
    expect(readme).toContain('##### D. 선택: 데이터와 공통 step');
    ...
    expect(readme).toContain('- endpoint 변경: `scenarios/*.yaml`의 `api.path`');
    ...
    expect(readme).toContain('Start with plain steps first; use `vars`, `fixtures`, or `include` only when values or steps repeat.');
    expect(readme).toContain('For environment-specific data, use scenario `fixtures:` or pass `--var-file`/`--var` to `validate`, `generate`, `test`, or `run`.');
    expect(readme).toContain('Reuse common login/seed flows with `- include: ./partials/login.yaml`; include files stay under the entry scenario directory and must not define `vars:` or `fixtures:`.');
    ...
    expect(readme).toContain('통과한 scenario만 npx --yes openapi-k6 run -s <name> --log -- --vus 1 --iterations 1 형식으로 짧게 실행해.');
    expect(readme).toContain('스크립트만 필요하면 npx --yes openapi-k6 generate -s <name> 형식으로 k6 스크립트를 생성해.');
