...
describe('openapi-k6 CLI', () => {
  ...
  it('serves a local UI for listing scenarios and streaming validate output', async () => {
    ...
    try {
      ...
      expect(html).not.toContain('id="runHistory"');
      expect(html).not.toContain('renderRunHistory');
      expect(html).toContain('state.activeOutputRunId');
      expect(html).toContain('runsByScenario: new Map()');
      expect(html).toContain('saveScenarioRun(runItem);');
      expect(html).toContain('const items = [runs.validate, runs.test].filter(Boolean);');
      ...
      expect(html).toContain('id="connectionOverlay"');
      expect(html).toContain('UI 서버 연결 끊김');
      expect(html).toContain('body.ui-disconnected header');
      expect(html).toContain('body.ui-disconnected main');
      expect(html).toContain('filter: blur(2px);');
      expect(html).toContain('setUiDisconnected');
      expect(html).toContain('clearUiDisconnected');
      expect(html).toContain('UiConnectionError');
      expect(html).toContain('UI_CONNECTION_CHECK_INTERVAL_MS');
      ...
      expect(html).toContain('state.openStepIndexes.has(nextIndex)');
      expect(html).toContain('state.openStepIndexes.add(nextIndex)');
