...
describe('openapi-k6 CLI', () => {
  ...
  it('initializes a openapi-k6 scaffold in the target project', async () => {
    ...
    expect(readme).toContain('UI는 `scenarios/` 아래 폴더를 그룹으로 보여주고, 요청 단계에서 각 step의 출처를 `직접 정의`, `시나리오 사용: auth/login`, `파일 포함: ...`로 표시합니다.');
    expect(readme).toContain('`test` 실행 결과는 최근 실행 결과에서 단계별 성공/실패, HTTP status, 소요시간, 출처를 함께 보여줍니다.');
    expect(readme).toContain('UI에서 폴더는 접고 펼칠 수 있으며, 재사용된 step은 요청 단계와 최근 실행 결과에서 `시나리오 사용: auth/login` 또는 `파일 포함: ...` 출처가 표시됩니다.');
