...
- 적용 기준: viewport 너비 `1200px` 초과
- 탐색 패널: 기본 300, 최소 260, 최대 480
- 실행 패널: 기본 420, 최소 360, 최대 640
- 흐름 패널: 남은 공간을 사용하며 양쪽 패널을 넓히면 함께 줄어든다.
- 패널 너비는 민감하지 않은 UI 설정으로 localStorage에 저장한다.

세 패널의 기본 너비 합이 약 `1200px`이므로 기본 흐름 폭을 확보하도록 전환 기준을
`1200px`로 조정했다.

### 모바일·좁은 화면

- 적용 기준: viewport 너비 `1200px` 이하
- 세 패널을 세로로 길게 쌓지 않는다.
...
요청·응답 값은 기본으로 숨긴다. 각 endpoint 행의 펼침 chevron으로 이 영역을 연다. 민감 값은 실제 표시 모드에서도 기존 마스킹 규칙을 유지한다.
...
| 위치 | 저장 항목 | 저장하지 않는 항목 |
| --- | --- | --- |
| localStorage | 패널 너비 | 입력값, 요청·응답, 로그, secret |
...
- 패널 너비를 localStorage에 저장한다.

Astryx `TreeList`는 개별 확장 변경 callback을 제공하지 않는다. 전체 접기·펼치기는
현재 세션에서 지원하고, 개별 그룹 접힘 상태는 별도 저장하지 않는다.
...
검증(2026-07-30): `pnpm typecheck`, 233개 테스트, `pnpm build`, E2E·호환성·UI 자산
smoke와 1440px/639px 브라우저 확인을 통과했다.
...
검증(2026-07-30): 요청·예상 응답 formatter와 YAML literal secret·배열·객체 마스킹을
포함한 238개 테스트, `pnpm typecheck`, `pnpm build`, E2E·호환성·UI 자산 smoke,
`npm pack --dry-run`, 단일·재사용·멀티 모듈·snapshot 누락 브라우저 확인
(데스크톱/639px)을 통과했다.
...
검증(2026-07-30): 실행 상태 reducer, SSE ID·증분 replay, 입력 재연결, 실행 결과·로그
민감값 마스킹을 포함한 250개 테스트, `pnpm run typecheck`, `pnpm run build`, E2E·호환성·
UI 자산 smoke, `npm pack --dry-run`을 통과했다. 브라우저에서 성공·첫 단계 실패·
validate/test 결과 분리·민감 입력 제출과 실행 후 마스킹을 데스크톱/639px로 확인했다.
