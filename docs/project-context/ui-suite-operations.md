...
`openapi-k6 ui`는 browser를 자동으로 열지 않고 로컬 HTTP 서버 URL을 출력한다. 기본 bind는 `127.0.0.1:3766`이며 기본 포트가 사용 중이면 인접 포트를 제한적으로 시도한다. 서버는 scenario/suite 목록·상세, 실행, report 조회·다운로드, 상태 점검 route를 제공한다. 근거: [UI server](../../src/cli/ui/server.ts), [HTTP lifecycle](../../src/cli/ui/server-http.ts).
...
PR/main CI는 Node 20에서 install, typecheck, test, build, E2E, compatibility smoke, pack을 모두 실행한다. `vX.Y.Z` publish는 tag commit이 main에 포함되고 tag와 package version이 일치해야 하며 Node 24에서 typecheck, test, build, E2E, pack 후 npm publish한다. Publish job에는 `test:compat`가 없고, 성공 뒤 published smoke가 별도 실행된다. 근거: [CI workflow](../../.github/workflows/ci.yml), [publish workflow](../../.github/workflows/publish.yml), [published smoke workflow](../../.github/workflows/published-smoke.yml).

suite/report/UI는 Vitest 통합 테스트가 주 검증이다. 현재 E2E·compatibility·published smoke는 이 기능을 직접 실행하지 않는다. UI 육안 확인은 build 후 `pnpm run sample:ui`로 임시 workspace를 띄워 수행한다.
