...
이 문서는 현재 아키텍처와 설계 결정을 정리한다. 사용자 실행 방법은 루트 `README.md`, 기능별 구현 계약은 `docs/feature-breakdown.md`를 기준으로 한다.
...
기본 사용 흐름에서는 `openapi-k6 init`으로 `load-tests/config.yaml`을 만들고, 사용자가 `TODO` 값을 채운 뒤 `openapi-k6 sync`, `openapi-k6 generate -s smoke`를 실행한다. CLI는 남은 `TODO` 값을 발견하면 실행을 중단한다.
...
기본 경로가 아니거나 특정 module을 지정해야 할 때만 `--config`, `--module`을 명시한다.
...
- Scenario DSL parser
- OpenAPI parser / API registry / resolver
- AST builder
- k6 generator
- OpenAPI snapshot/catalog sync
- `load-tests/config.yaml` 기반 module 선택
- 대상 프로젝트 `load-tests` init scaffold
- fixture 기반 테스트
...
기본 멀티모듈 설정과 `--module` 선택은 구현되어 있다. `generate` 1회 실행은 1개 module registry를 사용한다.
...
| CLI 명령 형태 | `openapi-k6 generate -s smoke` | 기본 `load-tests/config.yaml`과 `defaultModule`을 사용한다. `smoke`는 `load-tests/scenarios/smoke.yaml`로 해석되고 출력은 `load-tests/generated/smoke.k6.js`가 기본값이다. |

## 19. 후속 구현 후보

1. UI flow adapter 설계와 구현
2. Swagger/OpenAPI URL 자동 탐색
3. auth scheme 감지와 header template 제안
4. `loop`, `retry`, `timeout` step 옵션
5. k6 실행 wrapper
