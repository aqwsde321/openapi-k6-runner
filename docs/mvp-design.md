...
`dev.openapi.json`은 원격 OpenAPI URL에서 받은 snapshot이다. `catalog.json`은 사람이 scenario YAML을 작성할 때 참고하는 endpoint 목록이며, compiler의 필수 입력은 아니다.
...
- 후속 CLI는 `--module bos`처럼 module 선택 옵션을 제공한다.
...
| CLI 명령 형태 | `openapi-k6 generate -s scenario.yaml -o openapi.yaml -w output.js` | 입력/출력 책임이 명확하다. base URL은 `.env`에서 읽는다. |
