...
이 문서는 구현 기능의 계약을 정리한다. 실제 실행 방법은 루트 [README](../../README.md)를 기준으로 하고, 구현 순서 이력은 [작업 계획](../planning/work-plan.md)에만 남긴다.
...
- CLI `--var-file`과 `--var`는 `validate`, `generate`, `test`, `run` 실행 시점에 scenario vars를 override할 수 있다.
...
`module` 하위 명령은 config 수동 편집 없이 module 등록, 목록 확인, 기본 module 전환을 지원한다. `module add`는 `--openapi`를 명시할 수 있고, 생략하면 `--base-url`에서 흔한 OpenAPI 경로를 자동 탐색한다.

```text
openapi-k6 module add auth --base-url https://auth-api.example.com --sync
...
- `.env.example`에는 secret placeholder만 있고, `load-tests/.gitignore`는 기본적으로 `scenarios/**`만 git 추적 대상에 남긴다.
...
- request 안의 `{{vars.NAME}}` template은 entry scenario의 `vars.NAME`, `fixtures`로 로드된 값, 또는 CLI `--var-file`/`--var` override로 정의되어 있어야 한다.
