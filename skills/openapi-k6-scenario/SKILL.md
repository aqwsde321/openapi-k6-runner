...
3. Never rerun `init` in an existing workspace. If `load-tests/config.yaml` exists and `openapi-k6/config.yaml` does not, do not expect `update` to move it automatically. Ask before running `mv load-tests openapi-k6`, and suggest it only when the `openapi-k6/` directory does not already exist. Otherwise resolve the directory conflict first. To keep the legacy path, append `--config load-tests/config.yaml` to workspace commands.
